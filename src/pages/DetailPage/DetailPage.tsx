...
export default function DetailPage() {
  ...
  const safeMediaId = mediaId ? parseInt(mediaId) : undefined;
  ...
  // 내가 찜한 리스트 관련 훅
  const { isInList, toggleList } = useMyList(safeMediaType, safeMediaId);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !ratingContainerRef.current.contains(event.target as Node)
      ) {
  ...
  const runtime =
    content?.runtime ??
    (content?.episode_run_time ? content.episode_run_time[0] : 0);
            ...
            >
              ...
              <S.ActionButtons>
                ...
                >
                  ...
                  <S.CloseRatingButton
                    onClick={() => setIsRatingMenuOpen(false)}
                  >
                    ✕
                  </S.CloseRatingButton>
                </S.RatingContainer>
