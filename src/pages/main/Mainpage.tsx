...
// 타입 정의 및 헬퍼 함수들
type Media = { id: number; title?: string; poster_path?: string | null; backdrop_path?: string | null; media_type?: "movie" | "tv"; };
type RowData = { id: string; title: string; items: Media[]; };
type TMDBMovieRaw = { id: number; title?: string; original_title?: string; poster_path?: string | null; backdrop_path?: string | null; };
type TMDBTVRaw = { id: number; name?: string; original_name?: string; poster_path?: string | null; backdrop_path?: string | null; };
type TMDBListResponse<T> = { page: number; results: T[]; total_pages: number; total_results: number; };
const posterURL = (path: string | null | undefined, size: "w154" | "w342" | "w500" = "w342") => (path ? `https://image.tmdb.org/t/p/${size}${path}` : "");
const takeDistinct = <T extends { id: number }>(list: T[], count: number, banned: Set<number>): T[] => {
  const out: T[] = [];
...
const mapMovie = (m: TMDBMovieRaw): Media => ({ id: m.id, title: m.title ?? m.original_title ?? "제목 없음", poster_path: m.poster_path ?? null, backdrop_path: m.backdrop_path ?? null, media_type: "movie", });
const mapTV = (t: TMDBTVRaw): Media => ({ id: t.id, title: t.name ?? t.original_name ?? "제목 없음", poster_path: t.poster_path ?? null, backdrop_path: t.backdrop_path ?? null, media_type: "tv", });
...
export default function Home(): React.JSX.Element {
  ...
  useEffect(() => {
    const api = axios.create({ baseURL: "https://api.themoviedb.org/3", params: { api_key: import.meta.env.VITE_TMDB_API_KEY, language: "ko-KR" }, });
    (async () => {
      try {
        const [discMovieRes, discTVRes] = await Promise.all([
          api.get<TMDBListResponse<TMDBMovieRaw>>("/discover/movie", { params: { with_original_language: "ko", sort_by: "popularity.desc", region: "KR" }, }),
          api.get<TMDBListResponse<TMDBTVRaw>>("/discover/tv", { params: { with_original_language: "ko", sort_by: "popularity.desc" }, }),
        ]);
        const discMovies = discMovieRes.data.results.map(mapMovie);
        const discTVs = discTVRes.data.results.map(mapTV);
        const nowPlayingRes = await api.get<TMDBListResponse<TMDBMovieRaw>>("/movie/now_playing", { params: { region: "KR" } });
        const nowPlaying = nowPlayingRes.data.results.map(mapMovie);
        const [moviePopularRes, tvPopularRes] = await Promise.all([
          api.get<TMDBListResponse<TMDBMovieRaw>>("/movie/popular", { params: { region: "KR" }, }),
  ...
  const rows: RowData[] = useMemo(() => [
    ...
    ...(myList.length > 0 ? [{ id: "r2", title: "내가 찜한 리스트", items: myList } as RowData] : []),
  ...
  const crayonShinchanMovieId = "361309"; // 짱구: 나의 이사 이야기 영화 ID
    ...
    <S.Page>
      ...
      <S.Hero>
        ...
        <S.HeroContent>
          ...
          <S.BtnRow>
            <S.PlayBtn 
              href={`https://www.themoviedb.org/movie/${crayonShinchanMovieId}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              ▶ 재생
            </S.PlayBtn>
            <S.InfoBtn to={`/home/movie/${crayonShinchanMovieId}`}>
              ⓘ 상세 정보
            </S.InfoBtn>
          </S.BtnRow>
        </S.HeroContent>
      ...
      <S.RowSection>
        {rows.map((row) => (
          <S.Row key={row.id}>
            ...
            <S.SliderWrapper>
              <S.ArrowLeft aria-label="left" onClick={() => handleScroll(row.id, -600)} style={{ left: 8 }}>◀</S.ArrowLeft>
              <S.Slider
                id={row.id}
                ref={(el) => {
              ...
              <S.ArrowRight aria-label="right" onClick={() => handleScroll(row.id, 600)}>▶</S.ArrowRight>
            </S.SliderWrapper>
          </S.Row>
        ))}
      </S.RowSection>
