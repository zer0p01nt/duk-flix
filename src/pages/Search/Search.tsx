...
export default function Search({
  ...
  useEffect(() => {
    ...
    fetch(`${TMDB_BASE}/search/multi?api_key=${TMDB_KEY}&language=ko-KR&query=${encodeURIComponent(q)}&page=1&include_adult=false`, { signal: ctrl.signal })
      .then(res => {
      ...
      <S.HeaderBar>
        ...
        <S.Ul>
          <S.Li ref={dropdownRef}>
            <S.SearchNav onClick={() => setOpen((prev) => !prev)}>메뉴 ▼</S.SearchNav>
            {open && (
              <S.Dropdown>
                <S.DropdownIcon></S.DropdownIcon>
                <S.DropdownUl>
                  <S.DropdownLi onClick={() => navigate("/home")} $isSelected={location.pathname === "/home"}>홈</S.DropdownLi>
                  <S.DropdownLi onClick={() => navigate("/series")} $isSelected={location.pathname === "/series"}>시리즈</S.DropdownLi>
                  <S.DropdownLi onClick={() => navigate("/movies")} $isSelected={location.pathname === "/movies"}>영화</S.DropdownLi>
                  <S.DropdownLi onClick={() => navigate("/new")} $isSelected={location.pathname === "/new"}>NEW & 인기</S.DropdownLi>
                  <S.DropdownLi onClick={() => navigate("/my-list")} $isSelected={location.pathname === "/my-list"}>내가 찜한 리스트</S.DropdownLi>
                </S.DropdownUl>
              </S.Dropdown>
            )}
          </S.Li>
        ...
        <S.Nav>
          <S.NavItem onClick={() => navigate("/home")} $isSelected={location.pathname === "/home"}>홈</S.NavItem>
          <S.NavItem onClick={() => navigate("/series")} $isSelected={location.pathname === "/series"}>시리즈</S.NavItem>
          <S.NavItem onClick={() => navigate("/movies")} $isSelected={location.pathname === "/movies"}>영화</S.NavItem>
          <S.NavItem onClick={() => navigate("/new")} $isSelected={location.pathname === "/new"}>NEW & 인기</S.NavItem>
          <S.NavItem onClick={() => navigate("/my-list")} $isSelected={location.pathname === "/my-list"}>내가 찜한 리스트</S.NavItem>
        </S.Nav>
        <S.HeaderActions>
          {showSearch ? (
          ...
          <div onMouseEnter={() => setIsProfileOpen(true)} onMouseLeave={() => setIsProfileOpen(false)}>
            <S.AvatarWrapper><S.AvatarBox><S.Avatar /><S.AvatarTriangle $isRotated={isProfileOpen} /></S.AvatarBox></S.AvatarWrapper>
            <S.ProfileDropdown $isOpen={isProfileOpen}>
              <S.ProfileDropdownIcon />
              <S.ProfileDropdownItem $isButton={false}><S.Avatar /><span>{userEmail}</span></S.ProfileDropdownItem>
              <S.ProfileDropdownHr />
              <S.ProfileDropdownItem as="button" onClick={handleSignOut} $isButton={true}>넷플릭스에서 로그아웃</S.ProfileDropdownItem>
            </S.ProfileDropdown>
          </div>
        </S.HeaderActions>
      </S.HeaderBar>
      ...
      <S.SearchPage>
        ...
        <S.main>
          {noResult ? (
            <S.text>
              <S.NoSearch>
                <S.NoSearchTitle>입력하신 검색어 '{query}'(와)과 일치하는 결과가 없습니다.</S.NoSearchTitle>
                <S.ReSearch>추천 검색어:</S.ReSearch>
                <S.ReSearchUl>
                  <S.ReSearchLi>다른 키워드를 입력해 보세요.</S.ReSearchLi>
                  <S.ReSearchLi>시리즈나 영화를 찾고 계신가요?</S.ReSearchLi>
                  <S.ReSearchLi>영화 제목, 시리즈 제목, 또는 배우나 감독의 이름으로 검색해 보세요.</S.ReSearchLi>
                  <S.ReSearchLi>코미디, 로맨스, 스포츠 또는 드라마와 같은 장르명으로 검색해 보세요.</S.ReSearchLi>
                </S.ReSearchUl>
              </S.NoSearch>
            </S.text>
          ) : (
                    ...
                    {movies.map((m) => {
                      ...
                      return (
                        <Link key={m.id} to={`/search/${m.media_type}/${m.id}`}>
                          <S.Movie><S.Poster src={img} alt={title} title={title} /></S.Movie>
                        </Link>
                      );
                    })}
                  </S.MovieGrid>
